...
public class CanvasDAO {
    ...
    public HashMap<String, ArrayList<String>> retrieveAll() {
        ...
        try {
            conn = ConnectionManager.getConnection();
            String sql = "Select * from customer_acquisition"; //get all traits and details from first value driver. then repeat this for the rest, to add all into allMap.
            preStmt = conn.prepareStatement(sql);
            rs = preStmt.executeQuery();
            while (rs.next()) {
                String trait = rs.getString("choice");
    ...
    public HashMap<String, Integer> resultsFromAllTraitsSelected(ArrayList<String> selectedTraits, HashMap<String, ArrayList<String>> allData) {
        ...
        HashMap<String, ArrayList<String>> allDataFinal = new HashMap<String, ArrayList<String>>();
        Iterator iter = allData.entrySet().iterator();
        
        while(iter.hasNext()) {
             Map.Entry pair = (Map.Entry)iter.next();
             allDataFinal.put((String)pair.getKey(), (ArrayList<String>)pair.getValue());
             iter.remove();
         }
        
        for (String trait : selectedTraits) {
            /*if(allDataFinal.keySet().contains(trait)) {
                resultMap.put(trait, 1);
            }*/
          //  ArrayList<String> companiesWithTrait = allDataFinal.get(trait); //null pointer here. may be data doesn't tally.
            ArrayList<String> companiesWithTrait = new ArrayList<>();
            iter = allDataFinal.entrySet().iterator();
            
            while(iter.hasNext()) {
                Map.Entry pair = (Map.Entry)iter.next();
                if(pair.getKey().equals(trait)) {
                   // resultMap.put(trait, 1);
                   ArrayList<String> value = (ArrayList<String>)pair.getValue();
                   for(String v : value) {
                       companiesWithTrait.add(v);
                   }
                }
            }
    ...
    public HashMap<String, Integer> nearestSearchFromResults(HashMap<String, Integer> resultsFromAllTraitsSelected) {
        ...
        while (iter.hasNext()) {
            ...
            if((Integer)pair.getValue() == maxValue) {
                nearestResults.put((String)pair.getKey(),(Integer)pair.getValue());
            }
        }
        /*
        while(iter.hasNext()) {
